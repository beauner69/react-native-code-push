...
+ (NSString *)bundleAssetsPath {
  ...
  if (bundleResourceSubdirectory) {
    resourcePath = [resourcePath
        stringByAppendingPathComponent:bundleResourceSubdirectory];
  }

  return [resourcePath
      stringByAppendingPathComponent:[CodePushUpdateUtils assetsFolderName]];
}
...
+ (NSURL *)bundleURLForResource:(NSString *)resourceName
  ...
  NSDictionary *currentPackageMetadata =
      [CodePushPackage getCurrentPackage:&error];
  ...
  NSString *packageDate =
      [currentPackageMetadata objectForKey:BinaryBundleDateKey];
  NSString *packageAppVersion =
      [currentPackageMetadata objectForKey:AppVersionKey];

  if ([[CodePushUpdateUtils modifiedDateStringOfFileAtURL:binaryBundleURL]
          isEqualToString:packageDate] &&
      ([CodePush isUsingTestConfiguration] ||
       [binaryAppVersion isEqualToString:packageAppVersion])) {
    // Return package file because it is newer than the app store binary's JS
    // bundle
...
+ (void)ensureBinaryBundleExists {
  if (![self binaryBundleURL]) {
    ...
    errorMessage =
        @"React Native doesn't generate your app's JS bundle by default when "
        @"deploying to the simulator. "
         "If you'd like to test CodePush using the simulator, you can do one "
         "of the following depending on your "
         "React Native version and/or preferred workflow:\n\n"

         "1. Update your AppDelegate.m file to load the JS bundle from the "
         "packager instead of from CodePush. "
         "You can still test your CodePush update experience using this "
         "workflow (Debug builds only).\n\n"

         "2. Force the JS bundle to be generated in simulator builds by adding "
         "'export FORCE_BUNDLING=true' to the script under "
         "\"Build Phases\" > \"Bundle React Native code and images\" (React "
         "Native >=0.48 only).\n\n"

         "3. Force the JS bundle to be generated in simulator builds by "
         "removing the if block that echoes "
         "\"Skipping bundling for Simulator platform\" in the "
         "\"node_modules/react-native/packager/react-native-xcode.sh\" file "
         "(React Native <=0.47 only)\n\n"

         "4. Deploy a Release build to the simulator, which unlike Debug "
         "builds, will generate the JS bundle (React Native >=0.22.0 only).";
#else
    errorMessage = [NSString
        stringWithFormat:@"The specified JS bundle file wasn't found within "
                         @"the app's binary. Is \"%@\" the correct file name?",
                         [bundleResourceName stringByAppendingPathExtension:
                                                 bundleResourceExtension]];
#endif
#else
    errorMessage = @"Something went wrong. Please verify if generated JS "
                   @"bundle is correct. ";
#endif
...
- (void)initializeUpdateAfterRestart {
  ...
  if (pendingUpdate) {
    ...
    if (updateIsLoading) {
      // Pending update was initialized, but notifyApplicationReady was not
      // called. Therefore, deduce that it is a broken update and rollback.
      CPLog(@"Update did not finish loading the last time, rolling back to a "
            @"previous version.");
...
+ (BOOL)isFailedHash:(NSString *)packageHash {
  ...
  if (failedUpdates == nil || packageHash == nil) {
    ...
    for (NSDictionary *failedPackage in failedUpdates) {
      // Type check is needed for backwards compatibility, where we used to just
      // store the failed package hash instead of the metadata. This only
      // impacts "dev" scenarios, since in production we clear out old
      // information whenever a new binary is applied.
      if ([failedPackage isKindOfClass:[NSDictionary class]]) {
        NSString *failedPackageHash =
            [failedPackage objectForKey:PackageHashKey];
...
+ (BOOL)isPendingUpdate:(NSString *)packageHash {
  ...
  // it "pending". Additionally, if a specific hash was provided, we ensure it
  // matches that of the pending update.
...
- (void)loadBundle {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if ([CodePush isUsingTestConfiguration] ||
        ![super.bridge.bundleURL.scheme hasPrefix:@"http"]) {
...
- (void)rollbackPackage {
  ...
  if (!failedPackage) {
    if (error) {
      CPLog(@"Error getting current update metadata during rollback: %@",
            error);
    } else {
 ...
 * so that it can be used when the actual update application occurs at a later
 * point.
 */
- (void)savePendingUpdate:(NSString *)packageHash isLoading:(BOOL)isLoading {
  ...
  NSUserDefaults *preferences = [NSUserDefaults standardUserDefaults];
  NSDictionary *pendingUpdate = [[NSDictionary alloc]
      initWithObjectsAndKeys:packageHash, PendingUpdateHashKey,
                             [NSNumber numberWithBool:isLoading],
                             PendingUpdateIsLoadingKey, nil];
...
- (void)applicationWillEnterForeground {
  ...
  if (_lastResignedDate) {
    durationInBackground =
        [[NSDate date] timeIntervalSinceDate:_lastResignedDate];
  }
...
- (void)applicationWillResignActive {
  ...
  if (_installMode == CodePushInstallModeOnNextSuspend &&
      [[self class] isPendingUpdate:nil]) {
    _appSuspendTimer =
        [NSTimer scheduledTimerWithTimeInterval:_minimumBackgroundDuration
                                         target:self
...
RCT_EXPORT_METHOD(downloadUpdate
                  : (NSDictionary *)updatePackage notifyProgress
                  : (BOOL)notifyProgress resolver
  ...
  if (binaryBundleURL != nil) {
    [mutableUpdatePackage
        setValue:[CodePushUpdateUtils
                     modifiedDateStringOfFileAtURL:binaryBundleURL]
  ...
  [CodePushPackage downloadPackage:mutableUpdatePackage
      expectedBundleFileName:[bundleResourceName stringByAppendingPathExtension:
                                                     bundleResourceExtension]
      publicKey:publicKey
      operationQueue:_methodQueue
      // The download is progressing forward
      progressCallback:^(long long expectedContentLength,
                         long long receivedContentLength) {
        // Update the download progress so that the frame observer can notify
        // the JS side
      ...
      doneCallback:^{
        NSError *err;
        NSDictionary *newPackage =
            [CodePushPackage getPackage:mutableUpdatePackage[PackageHashKey]
                                  error:&err];

        if (err) {
          return reject([NSString stringWithFormat:@"%lu", (long)err.code],
      ...
      failCallback:^(NSError *err) {
        ...
        _didUpdateProgress = NO;
        self.paused = YES;
        reject([NSString stringWithFormat:@"%lu", (long)err.code],
               err.localizedDescription, err);
      }];
}
 ...
 * app version, as well as the deployment key that was configured in the
 * Info.plist file.
 */
RCT_EXPORT_METHOD(getConfiguration
  ...
  if (isRunningBinaryVersion) {
    // isRunningBinaryVersion will not get set to "YES" if running against the
    // packager.
    NSString *binaryHash =
        [CodePushUpdateUtils getHashForBinaryContents:[CodePush binaryBundleURL]
                                                error:&error];
    if (error) {
    ...
    if (binaryHash == nil) {
      // The hash was not generated either due to a previous unknown error or
      // the fact that the React Native assets were not bundled in the binary
      // (e.g. during dev/simulator) builds.
      resolve(configuration);
      return;
    }
 ...
 */
RCT_EXPORT_METHOD(getUpdateMetadata
                  : (CodePushUpdateState)updateState resolver
                  : (RCTPromiseResolveBlock)resolve rejecter
                  : (RCTPromiseRejectBlock)reject) {
  NSError *error;
  NSMutableDictionary *package =
      [[CodePushPackage getCurrentPackage:&error] mutableCopy];
  ...
  // We have a CodePush update, so let's see if it's currently in a pending
  // state.
  BOOL currentUpdateIsPending =
      [[self class] isPendingUpdate:[package objectForKey:PackageHashKey]];

  if (updateState == CodePushUpdateStatePending && !currentUpdateIsPending) {
    // The caller wanted a pending update
    // but there isn't currently one.
    resolve(nil);
  } else if (updateState == CodePushUpdateStateRunning &&
    ...
    if (isRunningBinaryVersion) {
      // This only matters in Debug builds. Since we do not clear "outdated"
      // updates, we need to indicate to the JS side that somehow we have a
      // current update on disk that is not actually running.
...
RCT_EXPORT_METHOD(installUpdate
                  : (NSDictionary *)updatePackage installMode
                  : (CodePushInstallMode)installMode minimumBackgroundDuration
                  : (int)minimumBackgroundDuration resolver
                  : (RCTPromiseResolveBlock)resolve rejecter
                  : (RCTPromiseRejectBlock)reject) {
  NSError *error;
  ...
  if (error) {
    ...
    [self savePendingUpdate:updatePackage[PackageHashKey] isLoading:NO];

    _installMode = installMode;
    if (_installMode == CodePushInstallModeOnNextResume ||
        _installMode == CodePushInstallModeOnNextSuspend) {
 ...
 * module, and is only used internally to populate the
 * RemotePackage.failedInstall property.
 */
RCT_EXPORT_METHOD(isFailedUpdate
...
/*
 * This method isn't publicly exposed via the "react-native-code-push"
 * module, and is only used internally to populate the LocalPackage.isFirstRun
 * property.
 */
RCT_EXPORT_METHOD(isFirstRun
  ...
  NSError *error;
  BOOL isFirstRun =
      _isFirstRunAfterUpdate && nil != packageHash &&
      [packageHash length] > 0 &&
      [packageHash
          isEqualToString:[CodePushPackage getCurrentPackageHash:&error]];

  resolve(@(isFirstRun));
}

/*
 * This method is the native side of the CodePush.notifyApplicationReady()
 * method.
 */
 ...
 * It is needed to switch to a different deployment if the current deployment is
 * more recent. Note: we don’t recommend to use this method in scenarios other
 * than that (CodePush will call this method automatically when needed in other
 * cases) as it could lead to unpredictable behavior.
 */
...
/*
 * This method is the native side of the
 * CodePush.downloadAndReplaceCurrentBundle() method, which replaces the current
 * bundle with the one downloaded from removeBundleUrl. It is only to be used
 * during tests and no-ops if the test configuration flag is not set.
 */
RCT_EXPORT_METHOD(downloadAndReplaceCurrentBundle
...
/*
 * This method is checks if a new status update exists (new version was
 * installed, or an update failed) and return its details (version label,
 * status).
 */
RCT_EXPORT_METHOD(getNewStatusReport
                  : (RCTPromiseResolveBlock)resolve rejecter
                  : (RCTPromiseRejectBlock)reject) {
  if (needToReportRollback) {
    ...
    NSDictionary *retryStatusReport =
        [CodePushTelemetryManager getRetryStatusReport];
